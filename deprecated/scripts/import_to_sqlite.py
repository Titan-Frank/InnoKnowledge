...
def ensure_unique_records(
    records: list[dict[str, Any]], label: str, key_field: str = "id"
) -> None:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "output_root", help="Versioned output root, for example data/v4"
    )
    parser.add_argument(
        "--db",
...
def import_snapshot(
    connection: sqlite3.Connection, dataset_id: str, snapshot: Any
) -> Counter:
...
def main() -> int:
    ...
    conflicts = find_conflicting_dataset_ids(
        connection, dataset_id, version_key, str(output_root)
    )
    ...
    with connection:
        ...
        insert_dataset(
            connection, dataset_id, version_key, str(output_root), args.notes
        )
