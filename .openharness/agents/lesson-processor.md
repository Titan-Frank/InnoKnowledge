...
Call `Skill({ skill: "chapter-extract" })` to load the extraction skill, then follow its instructions.
...
For each new backbone node, spawn `node-expander` via SubAgent.
...
```python
SubAgent(
    description=f"expand-{node_id}",
    prompt=f'''
    Expand one backbone node into a provisional node card.
    Node: {node_id}
    Lesson evidence: {evidence_summary}
    Output root: {output_root}

    Return one node card payload.
    Do not write canonical tables directly.
    ''',
