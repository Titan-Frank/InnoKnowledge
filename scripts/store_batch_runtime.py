...
def main() -> int:
    ...
    inputs: list[tuple[str, str | None, callable]] = [
        ("query", args.queries_file, lambda rows: normalize_queries(rows, batch_anchor)),
        ("node", args.nodes_file, lambda rows: rows),
        ("profile", args.profiles_file, lambda rows: rows),
        ("mention", args.mentions_file, normalize_mentions),
        ("evidence", args.evidence_file, normalize_evidence),
        ("node_card", args.node_cards_file, lambda rows: rows),
        ("relation_proposal", args.relation_proposals_file, normalize_relation_proposals),
    ]

    stats: Counter[str] = Counter()
    with connection:
        for record_type, file_arg, normalizer in inputs:
            if not file_arg:
                continue
            records = load_jsonl(Path(file_arg).expanduser().resolve())
