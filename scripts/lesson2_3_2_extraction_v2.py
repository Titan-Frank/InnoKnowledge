...
def insert_profile(
    ...
    cur.execute(
        ...
        (dataset_id, id, node_id, subject, school_stage, grade_band, context_key, curriculum_role,
         mastery_level, learning_objectives_json, textbook_refs_json, framework_refs_json, status, updated_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    """,
        (
            dataset_id,
...
def insert_evidence(
    ...
    cur = conn.cursor()
    source_path = (
        f"ocr/八年级/初中（五•四学制）_化学_沪科技版_全一册_八年级/hybrid_auto"
    )
    cur.execute(
        """
        INSERT OR REPLACE INTO evidence
        (dataset_id, id, source_type, source_id, anchor_ref, source_path, excerpt, locator,
         page_start, page_end, modality, extraction_method)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    """,
        (
            dataset_id,
...
def insert_edge(
    ...
    cur.execute(
        ...
        (dataset_id, id, edge_type, edge_layer, backbone_expand, from_id, to_id, directionality,
...
def insert_node_card(
    ...
    cur.execute(
        ...
        (dataset_id, node_id, id, card_layer, title, summary, sections_json, status, updated_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    """,
        (
            dataset_id,
...
def main():
    ...
    evidence_list = [
        ...
        (
            ...
            "在通常状况下，二氧化碳是一种无色、无气味的气体。在加压、降温条件下将二氧化碳液化，可得到液态二氧化碳。进一步加压、降温即可得到固态二氧化碳。固态二氧化碳外形与冰相似，但在常压下，当温度高于-78.5°C时，就会升华，直接变成气态二氧化碳，因此固态二氧化碳常被称为干冰。",
            "第66页，二氧化碳的物理性质",
            66,
            66,
        ),
    ...
    for e_id, src, tgt, edge_type, layer, expand, src_refs in edges:
        insert_edge(
            conn, dataset_id, e_id, src, tgt, edge_type, layer, expand, src_refs
        )
    ...
    for e_id, src, tgt, edge_type, layer, expand, src_refs in support_edges:
        insert_edge(
            conn, dataset_id, e_id, src, tgt, edge_type, layer, expand, src_refs
        )
        ...
        (
            ...
            [
                ...
                },
                {
                    "id": "misconception",
                    "title": "常见误解",
                    "content": "助燃不等于可燃。氧气能支持燃烧，但氧气本身不能燃烧，不是可燃物。",
                    "pattern_ref": "explanation/v2/misconception",
                },
            ],
        ),
