...
export function AnnotationWorkbench() {
  ...
  const selectedLessonEvidence = useMemo(
    () => data.evidence.filter((item) => item.lesson_id === selectedLessonId),
    [data.evidence, selectedLessonId],
  );
  ...
  const activeRows = useMemo(() => {
    const rows = data[activeCollection];
    if (!filter.trim()) return rows;
    const normalized = filter.trim().toLowerCase();
    return rows.filter((row) => JSON.stringify(row).toLowerCase().includes(normalized));
  }, [activeCollection, data, filter]);
  ...
  return (
    <main className="flex min-h-0 flex-1 flex-col bg-void">
      <div className="flex flex-wrap items-center gap-3 border-b border-border-subtle bg-surface px-4 py-3">
        <div className="flex min-w-0 flex-1 items-center gap-2">
          ...
          <div className="min-w-0">
            ...
            <div className="mt-0.5 text-[11px] text-text-muted">{data.metadata.dataset_id} · {data.metadata.version}</div>
          </div>
        </div>
      ...
      <div className="grid min-h-0 flex-1 grid-cols-1 lg:grid-cols-[320px_minmax(420px,1fr)_390px]">
        <aside className="min-h-0 overflow-y-auto border-r border-border-subtle bg-deep scrollbar-thin">
          ...
          <div className="max-h-44 overflow-y-auto border-b border-border-subtle p-2 scrollbar-thin">
            {data.lessons.length === 0 ? (
              ...
              <button
                ...
                className={`mb-1 flex w-full items-center justify-between rounded-md border px-2.5 py-2 text-left text-xs transition-colors ${
                  selectedLessonId === lesson.lesson_id
                    ? 'border-accent/40 bg-accent/15 text-text-primary'
                    : 'border-border-subtle bg-elevated text-text-secondary hover:bg-hover hover:text-text-primary'
                }`}
              >
                <span className="min-w-0 truncate">{lesson.title || lesson.lesson_id}</span>
                <span className="ml-2 shrink-0 text-[10px] text-text-muted">{lesson.lesson_id}</span>
              </button>
            ))}
          </div>
          ...
          <div className="space-y-2 p-3">
            ...
            <TextAreaField id="lesson-source-text" label="课本文本" value={lessonDraft.source_text} onChange={(value) => setLessonDraft((current) => ({ ...current, source_text: value }))} rows={12} />
        ...
        <section className="grid min-h-0 grid-rows-[minmax(260px,1fr)_minmax(220px,0.85fr)] border-r border-border-subtle bg-void">
              ...
              <div className="flex min-w-0 flex-1 items-center gap-2">
                ...
                <div className="min-w-0">
                  ...
                  <div className="mt-0.5 text-[10px] text-text-muted">
                    {selectedLessonId || lessonDraft.lesson_id} · {lessonDraft.page_start || '页码未填'}
                  </div>
                </div>
              </div>
              <div className="flex items-center gap-1">
                <ToolbarButton title="用选区生成证据草稿" onClick={fillEvidenceFromSelection} tone="accent" disabled={!selectedText}>
                  <Link className="h-3.5 w-3.5" />
                  证据草稿
                </ToolbarButton>
                <ToolbarButton title="用选区生成节点草稿" onClick={fillNodeFromSelection} disabled={!selectedText}>
                  <Database className="h-3.5 w-3.5" />
                  节点草稿
                </ToolbarButton>
                <ToolbarButton title="用选区生成不应抽取项草稿" onClick={fillNegativeFromSelection} disabled={!selectedText}>
                  <AlertCircle className="h-3.5 w-3.5" />
                  反例草稿
                </ToolbarButton>
              </div>
            </div>
            <div className="border-b border-border-subtle bg-deep px-3 py-2 text-[11px] text-text-muted">
              {selectedText ? (
                <span>当前选区：<span className="text-text-secondary">{previewText(selectedText, 140)}</span></span>
              ) : (
                <span>在下方原文中选中文字，再生成证据、节点或反例草稿。</span>
              )}
            </div>
            <div
              onMouseUp={captureSelection}
              onKeyUp={captureSelection}
              className="h-full max-h-[calc(100vh-24rem)] min-h-[220px] overflow-auto bg-void p-3 text-xs leading-6 text-text-primary scrollbar-thin"
            >
              {sourceText.trim() ? (
                ...
                <div className="flex h-full min-h-[220px] items-center justify-center rounded-md border border-dashed border-border-default text-xs text-text-muted">
                  在左侧“课本文本”里粘贴当前课时内容，然后保存或直接开始选取。
                </div>
              )}
            </div>
          </div>
          ...
          <div className="grid min-h-0 grid-rows-[auto_1fr]">
            <div className="flex flex-wrap items-center gap-2 border-b border-border-subtle bg-surface px-3 py-2">
              ...
              <label htmlFor="gold-filter" className="ml-auto flex min-w-[190px] items-center gap-2 rounded-md border border-border-subtle bg-elevated px-2.5 py-1.5">
            ...
            <div className="min-h-0 overflow-auto p-3 scrollbar-thin">
              {activeRows.length === 0 ? (
                <div className="flex min-h-[160px] items-center justify-center rounded-md border border-dashed border-border-default text-xs text-text-muted">
                  暂无记录
                </div>
              ) : (
        ...
        <aside className="min-h-0 bg-deep">
          <div className="flex items-center justify-between border-b border-border-subtle px-3 py-2">
            <div className="flex items-center gap-2 text-xs font-semibold text-text-primary">
              {activeCollection === 'nodes' ? <Database className="h-4 w-4 text-accent" /> : activeCollection === 'edges' ? <GitBranch className="h-4 w-4 text-accent" /> : <Link className="h-4 w-4 text-accent" />}
              {COLLECTION_META[activeCollection].label}
            </div>
            <div className="flex items-center gap-1">
          ...
          <div className="max-h-[calc(100vh-9rem)] overflow-y-auto p-3 scrollbar-thin">
            {!selectedLesson && data.lessons.length === 0 && (
            ...
            {activeCollection === 'nodes' && (
              <div className="space-y-3">
                ...
                <TextAreaField id="node-aliases" label="别名" value={joinList(nodeDraft.aliases)} onChange={(value) => setNodeDraft((current) => ({ ...current, aliases: splitList(value) }))} rows={3} />
                ...
                <TextAreaField id="node-evidence" label="证据编号" value={joinList(nodeDraft.evidence_ids)} onChange={(value) => setNodeDraft((current) => ({ ...current, evidence_ids: splitList(value) }))} rows={3} />
            ...
            {activeCollection === 'edges' && (
              <div className="space-y-3">
                ...
                <TextAreaField id="edge-direction" label="方向说明" value={edgeDraft.direction_note} onChange={(value) => setEdgeDraft((current) => ({ ...current, direction_note: value }))} rows={3} />
                ...
                <TextAreaField id="edge-evidence" label="证据编号" value={joinList(edgeDraft.evidence_ids)} onChange={(value) => setEdgeDraft((current) => ({ ...current, evidence_ids: splitList(value) }))} rows={3} />
            ...
            {activeCollection === 'evidence' && (
              <div className="space-y-3">
                ...
                <TextAreaField id="evidence-excerpt" label="原文摘录" value={evidenceDraft.excerpt} onChange={(value) => setEvidenceDraft((current) => ({ ...current, excerpt: value }))} rows={6} />
                ...
                {selectedLessonEvidence.length > 0 && (
                  <div className="border-t border-border-subtle pt-3">
                    <div className="mb-2 text-[11px] font-medium text-text-muted">当前课时证据</div>
                    <div className="space-y-1">
                      {selectedLessonEvidence.map((item) => (
                        <button
                          type="button"
                          key={item.gold_evidence_id}
                          onClick={() => selectRow('evidence', item)}
                          className="block w-full truncate rounded border border-border-subtle bg-elevated px-2 py-1.5 text-left text-[11px] text-text-secondary transition-colors hover:bg-hover hover:text-text-primary"
                        >
                          {item.gold_evidence_id} · {item.excerpt || item.locator}
                        </button>
                      ))}
                    </div>
                  </div>
