...
export function importOcrBundle(input: {
  ...
  if (input.mode === "in_place") {
    if (!inspection.markdown_path) {
      throw new Error("In-place OCR import requires an existing Markdown file; use copy mode to generate compatible Markdown.");
    }
    return {
      status: "success",
      created: false,
      ...
      source_markdown_path: inspection.markdown_path,
      raw_markdown_path: inspection.markdown_path,
