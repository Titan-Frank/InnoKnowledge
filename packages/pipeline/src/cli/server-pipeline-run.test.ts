...
test("server pipeline runner executes TypeScript quality gate and canonical reducer after staging", async () => {
  ...
  const interdisciplinaryStage = result.stages.find((stage) => stage.id === "interdisciplinary_analysis");
  ...
  const interdisciplinaryCommand = interdisciplinaryStage?.output?.command as string[];
  ...
  assert.equal(interdisciplinaryStage?.status, "completed");
  assert.ok(interdisciplinaryCommand.some((part) => part.endsWith("interdisciplinary-analyze.js")));
  assert.ok(interdisciplinaryCommand.includes("--maximum-candidates"));
  ...
  assert.ok(commands.at(-11)?.some((part) => part.endsWith("staging-quality.js")));
  assert.ok(commands.at(-10)?.some((part) => part.endsWith("merge-staged-lessons.js")));
  assert.ok(commands.at(-9)?.some((part) => part.endsWith("normalize.js")));
  assert.ok(commands.at(-8)?.some((part) => part.endsWith("generate-node-bodies.js")));
  assert.ok(commands.at(-7)?.some((part) => part.endsWith("generate-pedagogical-profiles.js")));
  assert.ok(commands.at(-6)?.some((part) => part.endsWith("backfill-embeddings.js")));
  assert.ok(commands.at(-5)?.some((part) => part.endsWith("backfill-unit-embeddings.js")));
  assert.ok(commands.at(-4)?.some((part) => part.endsWith("interdisciplinary-analyze.js")));
  ...
  assert.ok(commands.slice(0, -11).every((command) => command.some((part) => part.endsWith("extract-lesson-openai.js"))));
});
