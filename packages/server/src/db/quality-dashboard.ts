...
export async function loadPipelineQualityPayload(
  ...
  const [
    ...
    sql<StagingEvidenceRow[]>`
      ...
      WHERE dataset_id = ${datasetId}
    `.catch(() => []),
    sql<{ lesson_run_id: string; raw_node_id: string }[]>`
      SELECT lesson_run_id, raw_node_id
    ...
    sql<CanonicalEvidenceRow[]>`
      SELECT id, source_id, anchor_ref, modality, properties_json
      FROM world_evidence
      WHERE dataset_id = ${datasetId}
    `.catch(() => []),
  ]);
...
function buildQualityDashboard(input: {
  ...
  const lessons: PipelineQualityLessonRow[] = input.lessonRows.map((lesson) => {
    ...
    const evidenceIds = new Set(qualityEvidence.map((row) => textValue(row.raw_evidence_id)).filter(Boolean));
