...
\vspace{0.55em}
\noindent{\footnotesize $^\dagger$ Repository snapshot based on merge commit \code{2a7dabb} and inspected on July 12, 2026. The report source, synthetic public demo, screenshots, and release documentation include working-tree changes prepared after that commit; an archival release should replace this note with the final release tag or commit. The early retrieval and grounded-generation runtime is implemented, while semantic planning, AI Tutor behavior, learning-outcome studies, version governance, and large-scale adjudicated benchmarks remain future work unless explicitly marked otherwise.}
...
The fixture and evaluator provide a development smoke check rather than a paper-grade benchmark: they cover one subject, only five questions, locally assigned node identifiers, and no independent adjudication. Additional local pilot and ablation scaffolds exist in the working environment, but they are not part of the tracked snapshot and do not contain a completed independent human-scoring record; this report therefore makes no comparative-performance claim from them.

\subsection{Artifact, Data, and Rights Boundary}

The implementation baseline for this draft is merge commit \code{2a7dabb}; the report source, public demo, screenshots, and release documentation are later working-tree changes. The archival report must identify a release commit or tag that contains both the implementation and the cited artifacts.
