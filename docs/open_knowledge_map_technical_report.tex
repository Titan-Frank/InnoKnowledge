...
When configured, the vision-model path classifies images into decisions such as core content, supporting content, decorative image, mismatch, or uncertain. The decision is stored in \code{properties_json.image_relevance}. Decorative and mismatched image evidence can be hidden from the consumption view, while uncertain images are preserved and shown in the review workflow.
...
\begin{enumerate}
  ...
  \item Staging writes still deserve stronger transaction guarantees in failure cases.
