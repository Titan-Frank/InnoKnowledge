...
% Compile with: python3 /Users/titan-frank/.codex/plugins/cache/openai-bundled/latex/0.2.4/scripts/compile_latex.py /Users/titan-frank/Documents/hsd/research/Open-Knowledge-Map/docs/open_knowledge_map_technical_report.tex --engine xelatex
...
The current repository has five prompt-bearing model paths and two append mechanisms:
...
\begin{itemize}
  ...
  \item P4: object-level grounded generation over retrieved \code{ApiUnit} context, implemented in the server runtime;
  \item P5: staging-quality retry repair instructions, generated by the pipeline runner and appended to P1 after blocked lesson/chunk runs.
\end{itemize}
...
P4 & Runtime grounded generation over retrieved \code{ApiUnit} context & \code{packages/server/src/runtime/grounded-generation.ts}\\
P5 & Staging-quality retry repair prompt appended to P1 & \code{packages/pipeline/src/cli/server-pipeline-run.ts}\\
\bottomrule
\end{tabularx}
...
硬约束：\par
1. 只输出 \code{nodes}、\code{evidence\_units}、\code{issues} 三个字段。\par
...
\subsection{P4: Runtime Grounded Generation}

P4 is the server-side grounded-generation prompt. It retrieves \code{ApiUnit} records, serializes a bounded context, and asks the model to answer in JSON.
...
\subsection{P5: Staging-Quality Retry Prompt}

P5 is generated only after staging-quality blocks a lesson/chunk. It is appended through \code{--prompt} for the affected retry command.

\begin{prompttranscript}
这是第 \{retryNumber\} 次质量失败后的自动重抽。\par
上一轮问题：\{issueText\}\par
请不要只返回证据。必须从当前 chunk 抽取与\{subjectContext\}对应的核心知识对象，并为每个节点补齐 \code{definition}、\code{domain\_profile}、\code{mention}、\code{node\_card} 和 evidence \code{source\_refs}。\par
即使当前文本以习题、探究、小结或图示为主，也要从题干、公式、图、表和说明中抽取概念、对象、属性、方法、规则、表示、过程、案例、材料或活动。\par
...
Domain projection & Teaching context is not stored as a new node kind. It is stored in \code{world_domain_profiles}, with \code{domain}, \code{school_stages_json}, \code{curriculum_roles_json}, and optional \code{properties_json.pedagogical_profile}.\\
...
\code{world_domain_profiles} & Domain, school-stage, curriculum-role, and pedagogical projection for an object without changing the object's top-level kind.\\
...
\code{retrieval_candidates} & Retrieval candidates associated with batch anchors and query ids. This supports extraction context and later object-level retrieval.\\
\bottomrule
\end{tabularx}
...
\code{domain_profiles} & Domain-specific teaching and curriculum projections for the node, including school stages, curriculum roles, source references, and pedagogical profile data.\\
...
\subsection{Runtime Response Contracts}

\code{GET /api/source/:key/unit/:node_id} returns \code{ApiUnit}. The newer runtime retrieval path returns \code{UnitRetrievalResponse}, where each hit includes score fields, match reasons, and a full \code{ApiUnit}. The grounded-generation path returns \code{GroundedGenerationResponse}, including the question, answer, valid citations, unsupported claims, used node ids, the retrieval response, grounding status, invalid citation details, cited evidence ids, and model name. These runtime paths are implemented as an early executable subset; broad benchmark evaluation remains future work.
...
\midrule
\code{staging-quality} & Lesson/chunk staged output: non-empty nodes and evidence, valid node kinds, definitions, domain profiles, mentions, node cards, evidence-backed source references, edge endpoints, edge types, directionality, and required card sections. & Blocks affected lesson/chunk runs unless invoked in warning-only mode. Blocked chunks can be retried with a generated repair prompt.\\
\code{strict-qa} & Canonical schema and evidence legality: valid node kinds, domains, learning modes, edge types, directionality, evidence references, required profiles, mentions, cards, card sections, node body format, body source references, and media references. & Returns \code{blocked} when errors exist.\\
...
\code{strict-qa} checks canonical data after merge and normalization. Its current error categories include \code{node}, \code{edge}, \code{domain_profile}, \code{mention}, \code{node_card}, \code{node_card_section}, and \code{node_body}. It checks, among other things:
...
\midrule
Zero-node chunk & \code{chunk:11-4-a} first returned no nodes, which is unusable even if evidence exists. & \code{staging-quality} blocks zero-node output; the server runner can retry blocked chunks with a repair prompt.\\
...
The current quality system validates whether a run is structurally usable and evidence-grounded enough for inspection. It does not yet prove that the graph is pedagogically optimal, that all relations are semantically correct, that object-level retrieval beats chunk retrieval, or that learners achieve better outcomes. Those claims require external benchmarks, human annotation, baseline comparisons, and learning-outcome evaluation.
