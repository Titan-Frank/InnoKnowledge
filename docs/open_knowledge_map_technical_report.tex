...
Figure~\ref{fig:architecture} shows the implemented construction and serving substrate, including the early runtime subset but not the full future Knowledge Runtime. The main flow starts from a PDF textbook or a previously generated MinerU markdown file. The pipeline creates or loads a textbook outline, splits the material into lessons or chunks, attaches optional enrich-book hints from PostgreSQL, calls model-based extraction, optionally asks a vision model to classify image evidence, writes staged records, applies quality gates, merges and normalizes canonical objects, generates knowledge bodies and school-stage pedagogical profiles, optionally backfills node and unit embeddings through an operator-configured endpoint, and runs strict QA, graph integrity, and quality-dashboard stages. PostgreSQL is the authoritative structured store. The Hono server reads from it and exposes graph bundles, complete units, text/vector/hybrid unit retrieval, grounded generation, pipeline status, quality diagnostics, image review, and textbook workbench routes. The React viewer consumes only these API contracts.
...
\code{packages/pipeline} & Extraction pipeline, MinerU integration, outline parsing, enrich-context loading, lesson extraction, staging, transactional merge and normalization, retrieval candidates, body, pedagogical-profile, and embedding generation, QA, and progress tracking.\\
\code{packages/server} & Hono API server, PostgreSQL query layer, graph and unit assembly, complete-unit retrieval, grounded generation and streaming, pipeline launch/status, review, and textbook routes.\\
\code{packages/viewer} & React/Vite graph viewer, G6 graph canvas, unit detail and grounded-answer panels, textbook tree, pipeline debug page, image review, annotation, and frontend API client.\\
...
Figure~\ref{fig:pipeline-flow} summarizes the stage boundary. Lesson workers write only \code{world_lesson_runs} and \code{world_staging_*}. After staging quality passes, reducer-controlled merge, remapping, and normalization perform canonical writes transactionally; strict QA and graph-integrity checks run afterward. The server runner records database checking, MinerU parsing, outline preparation, lesson planning, lesson staging, staging-quality retry, canonical commit, normalization, node-body generation, school-stage pedagogical-profile generation, node-embedding backfill, unit-embedding backfill, strict QA, graph integrity, and quality-dashboard generation as ordered stages. Pedagogical-profile and embedding stages can be explicitly skipped; embedding stages require an operator-configured endpoint.
...
\begin{tikzpicture}[
  ...
  \node[stage, below=of s11] (s12) {12. Strict QA\\graph integrity};
  \node[stage, below=of s12] (s13) {13. Quality\\dashboard};
...
The server is a Hono application in \code{packages/server}. It exposes health, metadata, graph bundles, unit loading, node cards, object-level retrieval, grounded generation, pipeline status, pipeline quality diagnostics, image review, annotation, asset serving, and textbook workbench routes.
...
The viewer is a React/Vite application built around four work areas:
...
Pipeline and server tests & Stage behavior, SQL writes, data-shape handling, runtime parsing, citation-identifier validation, streaming helpers, and focused regression cases. & General extraction or answer accuracy across subjects.\\
...
\begin{enumerate}
  ...
  \item Evidence conflict management, expert review queues, version governance, and learner feedback write-back are not yet complete.
...
\code{world_pipeline_jobs}, stages, events, workers & Server-launched pipeline state, progress, logs, stage status, events, and worker status.\\
...
\code{quality-dashboard} & Operational metrics: lesson counts, node counts, relation counts, evidence coverage, isolated-node ratio, disconnected components, pending image reviews, merge-review items, blocked lessons, and manual pending items. & Does not replace QA gates; it exposes inspection and triage metrics.\\
Manual review & Human decisions for uncertain image evidence and merge-review cases; reviewer judgment for node admission and relation quality. & Updates stored review state or guides follow-up reruns and merge decisions.\\
\bottomrule
\end{tabularx}
