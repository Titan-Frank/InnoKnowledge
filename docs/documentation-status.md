...
本文判断当前公开仓库中哪些文档代表 `ai-nks-v0.1` 顶层标准，哪些文档只是工程基线或发布材料。个人讨论稿、旧路线图和单次运行记录不纳入公开仓库，保存在本地忽略目录中。
...
5. “旧讨论材料里的 K-Unit JSON 示例就是当前 API。”
   - 应改为：“它们只是思想草稿；当前 API 契约以 `ApiUnit` 和 `docs/knowledge-unit-contract.md` 为准。”
