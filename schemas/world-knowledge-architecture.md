...
跨学科发现属于治理平面，不是事实关系层本身。扫描读取正式 `world_nodes`、`world_edges` 和证据，但只写：

- `world_interdisciplinary_runs`
- `world_interdisciplinary_candidates`

候选分为同一对象和关系两类。名称、别名、语义键、领域和标签只用于召回与排序；候选没有经过人工复核和 reducer 类应用步骤前，不能进入 `ApiUnit.relations`。关系批准必须选择现有教材证据；同一对象批准后执行身份归一，不创建替代合并的 `same_as` 边。

治理平面保证“发现可能性”和“确认事实”分离，也保证前端复核不会绕过 canonical 写入事务。完整实现契约见 `docs/interdisciplinary-knowledge-network.md`。
...
桥接标签可以提高跨学科候选召回率，但仍然只属于检索信号。两个节点共享标签，不等于它们存在某种正式关系。
